...
#include "coarseupdatenewtheta.h"
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    mxArray *U_C, *BDRY, *fVals, *OB, *U_C_new, *U_F_new, *UF, *WIND, *W, *finewind, *uNEW, *UFINEMIN, *Uk, *FW,*Uk1,  *Uk2, *uf1, *uf2, *ucoarse1, *Uk3,*THETA;
    double *U, *BC, *F, *ucoarse,*ufine, *Ufine, *obs,  *wind, *w, *fw, *fwind, *UNEW, *UFM, *UK, *UK1, *UK2, *UF1, *UF2, *Ucoarse1, *UK3,*theta;
    ...
    THETA = plhs[4] = mxCreateDoubleMatrix(M,M,mxREAL);
...
if (paraiter>=1){
    ...
    U=coarseupdate(U, UFM, UK1, F,BC,obs,N,K,0,0,wind, fwind,T0, sweepiter);


    for (p=1;p<K;p++)
    {
        U=coarseupdate(U,UFM, UK1, F,BC,obs,N,K,p,0,wind, fwind,T0, sweepiter);
        U=coarseupdate(U,UFM, UK1, F,BC,obs,N,K,0,p,wind, fwind,T0, sweepiter);
...
if (paraiter>=2){
    ...
    /*U=coarseupdatenewtheta(U, Ucoarse1,UF1,UF2,UK1,UK2,UK3, F,BC,obs,N,K,0,0,wind, fwind, sweepiter, eps, delta, x0,theta,1,0,0);


    for (p=1;p<K;p++)
    {
        U=coarseupdatenewtheta(U, Ucoarse1,UF1,UF2, UK1,UK2,UK3, F,BC,obs,N,K,p,0,wind, fwind, sweepiter, eps, delta, x0,theta,1,0,0);
        U=coarseupdatenewtheta(U, Ucoarse1,UF1,UF2, UK1,UK2,UK3, F,BC,obs,N,K,0,p,wind, fwind, sweepiter, eps, delta, x0,theta,1,0,0);
    ...
    for (int a=3; a<=paraiter; a++){
            ...
            U=coarseupdatenewtheta(U, Ucoarse1,UF1,UF2,UK1,UK2,UK3, F,BC,obs,N,K,0,0,wind, fwind, sweepiter, eps, delta, x0,theta,2,6,6);


            for (p=1;p<K;p++)
            {
                U=coarseupdatenewtheta(U, Ucoarse1,UF1,UF2, UK1,UK2,UK3, F,BC,obs,N,K,p,0,wind, fwind, sweepiter, eps, delta, x0,theta,2,6,6);
                U=coarseupdatenewtheta(U, Ucoarse1,UF1,UF2, UK1,UK2,UK3, F,BC,obs,N,K,0,p,wind, fwind, sweepiter, eps, delta, x0,theta,2,6,6);
    ...
    for (d=0; d<M*M; d++)
    {
        ufine[d]=UFM[d];//copy final fine and coarse functions to output
