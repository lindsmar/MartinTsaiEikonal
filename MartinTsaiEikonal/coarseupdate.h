...
double * coarseupdate(double *Ucoarse, double *Ufine, double * UK, double  * R, double *BC,double *obs, int N,int K,int p, int z, double *wind, double *fwind, double T, int sweepiter);
                                    ...
                                    if (marked[i+j*M]==0){
                                      if (consis){
                                        if (prevW==newwind){
                                          ...
                                          if (abs(unew-UK[i+j*M])<1e-20){// || abs(Ufine[i+j*M]-UK[i+j*M])<1e-13){
                                            utemp = Ufine[i + j * M];
                                          }
                                          else{
