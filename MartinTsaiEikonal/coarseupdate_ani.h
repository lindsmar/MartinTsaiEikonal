...
#ifndef EIKONALPROJECT_COARSEUPDATE_ani_H
#define EIKONALPROJECT_COARSEUPDATE_ani_H
//
...
double * coarseupdate_ani(double *Ucoarse, double *Ufine, double * UK, double  * cbar, double *BC,double *obs, int N,int K,int p, int z, double *wind, double *fwind, double T, int sweepiter);
                    ...
                    for (j = (s2 < 0 ? jstart1 : jstart2); (s2 < 0 ? j >= 0 : j <= M - 1); j += K * s2) {
                        ...
                        if (marked[i+j*M]==0){
                          if (consis){
                            if (prevW==newwind){
                              unew=tempmin;
                              if (abs(unew-UK[i+j*M])<1e-15){// || abs(Ufine[i+j*M]-UK[i+j*M])<1e-13)
                                utemp = Ufine[i + j * M];
                              }
                              else{
